...
class Position:
    ...
    def add(self, direction, amount):
        if direction == "forward": self.horizontal = self.horizontal + amount
        elif direction == "down": self.depth = self.depth + amount
        elif direction == "up": self.depth = self.depth - amount
        else: raise NotImplementedError("Unknown Command")
